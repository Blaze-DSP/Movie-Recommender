...
def Home(request):
    ...
    response = requests.post('http://127.0.0.1:80/user/predict', json={'input': user_feat})
    movies = []
    print(response)
...
def Similar(request, title):
    ...
    response = requests.post('http://127.0.0.1:80/genres/predict', json={'input': movie_feat, 'title': title})
    movies = []
    if response.status_code == 200:
        movies = response.json()['movies']
...
def Genres(request):
    if request.method == 'POST':
        ...
        response = requests.post('http://127.0.0.1:80/genres/predict', json={'input': movie_feat, 'title': ''})
        movies = []
        if response.status_code == 200:
            movies = response.json()['movies']
